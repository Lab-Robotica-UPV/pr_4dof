...
[priors,Mu,Sigma] = readGmmFile(strcat('../',directory,'/outdata/gmm.txt'));

Data=[];
for n=1:n_files
  Datatmp = load(['../' directory '/outdata/data' num2str(n,'%.2d') '_rescaled.txt'])';
  %Datatmp = load(['../indata/data' num2str(n,'%.2d') '.txt'])';
  Data = [Data , Datatmp];
end
nbVar = size(Datatmp,1);
nbData = size(Datatmp,2);

regr = load(['../' directory '/outdata/gmr_Mu.txt'])';
rgSigma = load(['../' directory '/outdata/gmr_Sigma.txt'])';
size(regr)
rgSigma = reshape(rgSigma,nbVar-1,nbVar-1,size(rgSigma,2));
...
for i=1:nbVar-1
    ...
    elseif nbVar == 5
        subplot(2,2,i)
    elseif nbVar == 6 || model.nbVar == 7
        subplot(3,2,i)
    end
  %subplot(2,nbVar-1,i+nbVar-1); 
  hold on; box on;
