...
namespace pr_modelling
{
    ...
    : Node("force_fixed_frame", options)
    {
        ...
        f_sub = this->create_subscription<pr_msgs::msg::PRForceState>(
            "force_state",
            1,
            std::bind(&ForceFixedFrame::topic_callback_force, this, std::placeholders::_1)
        );

        x_sub = this->create_subscription<pr_msgs::msg::PRArrayH>(
            "x_coord",
            1,
            std::bind(&ForceFixedFrame::topic_callback_x_mocap, this, std::placeholders::_1)
        );
    ...
    void ForceFixedFrame::topic_callback_force(const pr_msgs::msg::PRForceState::SharedPtr f_msg)
    {  
        if (init_x){
    ...
    void ForceFixedFrame::topic_callback_x_mocap(const pr_msgs::msg::PRArrayH::SharedPtr x_msg){
