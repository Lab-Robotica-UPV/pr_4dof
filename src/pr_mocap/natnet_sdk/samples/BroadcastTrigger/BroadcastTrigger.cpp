...
int receiverSocket;
struct sockaddr_in receive_addr, ToAddr;
struct hostent *server;
std::string ip_host = "158.42.21.85";
std::string start_message = "<CaptureStart><TimeCode VALUE=\"\"/><Name VALUE=\"RemoteTriggerTest_take01\"/><Notes VALUE=\"\"/><Description VALUE=\"\"/><DatabasePath VALUE=\"S:/shared/testfolder/\"/><PacketID VALUE=\"0\"/><ProcessID VALUE=\"%d\"/></CaptureStart>";;
std::string stop_message = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?><CaptureStop><TimeCode VALUE=\"12 13 14 15 0 0 1 1\"/><Name VALUE=\"RemoteTriggerTest_take01\"/><Notes VALUE=\"\"/><Description VALUE=\"\"/><DatabasePath VALUE=\"S:/shared/testfolder/\"/><PacketID VALUE=\"0\"/><ProcessID VALUE=\"%d\"/></CaptureStop>";
int value = 1;
#include <errno.h>
...
int main(){
    ...
    //server = gethostbyname(ip_host.c_str());
    //memcpy(&receive_addr.sin_addr, server->h_addr_list[0], server->h_length);
    ...
    // int err = connect(receiverSocket, (struct sockaddr *)&receive_addr, sizeof(struct sockaddr));
    // if (err == -1) {
    //       std:: cout << "Connection failed!" << std::endl;
    // }

    /*int err = bind(receiverSocket, (struct sockaddr *)&receive_addr, sizeof(struct sockaddr));
    if (err == -1) {
    ...
    char szMessage[512];
    sprintf(szMessage, start_message.c_str());
    int retval = sendto(receiverSocket, (char *)szMessage, 4 + strlen(szMessage), 0, (sockaddr *)&ToAddr, sizeof(ToAddr));

    //int retval = send(receiverSocket, (char *)start_message.c_str(), 4 + strlen(start_message.c_str()), 0);
    //int retval = sendto(receiverSocket, (char *)start_message.c_str(), 4 + strlen(start_message.c_str()), 0, (sockaddr *)&ToAddr, sizeof(ToAddr));
    if (retval == -1){
        std:: cout << "Send failed!" << std::endl;
        std::cout << errno << std::endl;
    }

    std::cout << "Finishing connection" << std::endl;
    return 0;
}
