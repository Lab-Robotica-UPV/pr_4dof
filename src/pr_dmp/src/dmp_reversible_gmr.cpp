#include "pr_dmp/dmp_reversible_gmr.hpp"
...
{
    /**** DmpRev COMPONENT ****/
    DmpRevGMR::DmpRevGMR(const rclcpp::NodeOptions & options)
    : Node("dmp_rev_gmr", options)
    {
        ...
        publisher_phase_speed_ = this->create_publisher<pr_msgs::msg::PRFloatH>(
			"dmp_phase_speed", 
			1);
        // Publisher of the error of the GMR
        publisher_error_gmr_ = this->create_publisher<pr_msgs::msg::PRFloatH>(
			"gmr_error", 
			1);
    ...
    void DmpRevGMR::force_callback(const pr_msgs::msg::PRArrayH::SharedPtr force_msg)
    {
        for(int i=0; i<4; i++)
    ...
    void DmpRevGMR::pos_callback(const pr_msgs::msg::PRArrayH::SharedPtr pos_msg)
    {
        ...
        auto gmr_error_msg = pr_msgs::msg::PRFloatH();
        gmr_error_msg.init_time = this->get_clock()->now();
        ...
        if (ref_read){
            ...
            if (!end_task){
                ...
                gmr_error_msg.header.stamp = pos_msg->header.stamp;
                gmr_error_msg.header.frame_id = pos_msg->header.frame_id;
                gmr_error_msg.data = error;
                ...
                gmr_error_msg.current_time = this->get_clock()->now();
                publisher_error_gmr_->publish(gmr_error_msg);
            }
        }
        else
        {
...
RCLCPP_COMPONENTS_REGISTER_NODE(pr_dmp::DmpRevGMR)