#include "pr_dmp/dmp_ref_gen.hpp"
...
{
    /**** DmpRefGen COMPONENT ****/
    DmpRefGen::DmpRefGen(const rclcpp::NodeOptions & options)
    : Node("dmp_ref_gen", options)
    {
        ...
        subscription_force_accel_ = this->create_subscription<pr_msgs::msg::PRArrayH>(
            "dmp_force_accel",
            1,
            std::bind(&DmpRefGen::force_accel_callback, this, _1));
        ...
        subscription_force_vel_ = this->create_subscription<pr_msgs::msg::PRArrayH>(
            "dmp_force_vel",
            1,
            std::bind(&DmpRefGen::force_vel_callback, this, _1));
        ...
        for (int i=0; i<n_dims;i++){
            damping_coefficient_vec(i) = damping_coefficient[i]/tau;
            spring_constant_vec(i) = spring_constant[i];
            mass_vec[i] = mass[i]/pow(tau,2);
        }
        ...
        // Initialize the DMP
        dmp = new Dmp(n_dims, function_approximators_dmp, damping_coefficient_vec, spring_constant_vec, mass_vec, alpha_phase);

        // Train the DMP. Passing the directory will make sure the results are saved to file.
        bool overwrite = true;
        dmp->train(trajectory_dmp,save_directory,overwrite);

        // Adjust the speed
        dmp->set_tau(tau/speed);
        ...
        s.resize(dmp->dim());
        sd.resize(dmp->dim());
        s_updated.resize(dmp->dim());
        y_gka.resize(dmp->dim());
        x_gka.resize(1);
        xd_gka.resize(1);
    ...
    void DmpRefGen::force_accel_callback(const pr_msgs::msg::PRArrayH::SharedPtr force_accel_msg)
    {
        for(int i=0; i<4; i++)
    ...
    void DmpRefGen::force_vel_callback(const pr_msgs::msg::PRArrayH::SharedPtr force_vel_msg)
    {
        for(int i=0; i<4; i++)
        ...
        if (ref_read){
            ...
            if (first_iter){
                ...
                if (phase_current[0] > phase_limit){
                    ...
                    dmp->integrateStepWithForceAndSlowDown(ts,s,s_updated,sd,force_accel,force_vel,slowdown);
                    s = s_updated;
                    s_pos = s.head(4);
                    if (gka_read){
...
RCLCPP_COMPONENTS_REGISTER_NODE(pr_dmp::DmpRefGen)