...
namespace pr_controllers
{
    ...
    : Node("gus_controller", options)
    {
        ...
        sub_pos = this->create_subscription<pr_msgs::msg::PRArrayH>(
            "joint_position",
            1,
            std::bind(&GusDiscretizedController::controller_callback, this, std::placeholders::_1)
        );
    ...
    void GusDiscretizedController::controller_callback(const pr_msgs::msg::PRArrayH::SharedPtr pos_msg)
    {
        if (init_ref){
